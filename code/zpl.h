...
#else
    extern "C++" {
        template <typename T> struct zplAlignment_Trick { char c; T member; };
#define zpl_align_of(Type) zpl_offset_of(zplAlignment_Trick<Type>, member)
    }
#endif
#endif
#else
...
#if defined(ZPL_COMPILER_MSVC)
    typedef struct zplAtomic32  { i32   volatile value; } zplAtomic32;
    typedef struct zplAtomic64  { i64   volatile value; } zplAtomic64;
    typedef struct zplAtomicPtr { void *volatile value; } zplAtomicPtr;
#else
#if defined(ZPL_ARCH_32_BIT)
...
#error Unknown architecture
    ...
    typedef struct zplAtomic32  { i32   volatile value; } __attribute__ ((aligned(4))) zplAtomic32;
    typedef struct zplAtomic64  { i64   volatile value; } __attribute__ ((aligned(8))) zplAtomic64;
    typedef struct zplAtomicPtr { void *volatile value; } __attribute__ ((aligned(ZPL_ATOMIC_PTR_ALIGNMENT))) zplAtomicPtr;
#endif
    
    ZPL_DEF i32  zpl_atomic32_load            (zplAtomic32 const volatile *a);
    ZPL_DEF void zpl_atomic32_store           (zplAtomic32 volatile *a, i32 value);
    ZPL_DEF i32  zpl_atomic32_compare_exchange(zplAtomic32 volatile *a, i32 expected, i32 desired);
    ZPL_DEF i32  zpl_atomic32_exchanged       (zplAtomic32 volatile *a, i32 desired);
    ZPL_DEF i32  zpl_atomic32_fetch_add       (zplAtomic32 volatile *a, i32 operand);
    ZPL_DEF i32  zpl_atomic32_fetch_and       (zplAtomic32 volatile *a, i32 operand);
    ZPL_DEF i32  zpl_atomic32_fetch_or        (zplAtomic32 volatile *a, i32 operand);
    ZPL_DEF b32  zpl_atomic32_spin_lock       (zplAtomic32 volatile *a, isize time_out); // NOTE: time_out = -1 as default
    ZPL_DEF void zpl_atomic32_spin_unlock     (zplAtomic32 volatile *a);
    ZPL_DEF b32  zpl_atomic32_try_acquire_lock(zplAtomic32 volatile *a);
    
    
    ZPL_DEF i64  zpl_atomic64_load            (zplAtomic64 const volatile *a);
    ZPL_DEF void zpl_atomic64_store           (zplAtomic64 volatile *a, i64 value);
    ZPL_DEF i64  zpl_atomic64_compare_exchange(zplAtomic64 volatile *a, i64 expected, i64 desired);
    ZPL_DEF i64  zpl_atomic64_exchanged       (zplAtomic64 volatile *a, i64 desired);
    ZPL_DEF i64  zpl_atomic64_fetch_add       (zplAtomic64 volatile *a, i64 operand);
    ZPL_DEF i64  zpl_atomic64_fetch_and       (zplAtomic64 volatile *a, i64 operand);
    ZPL_DEF i64  zpl_atomic64_fetch_or        (zplAtomic64 volatile *a, i64 operand);
    ZPL_DEF b32  zpl_atomic64_spin_lock       (zplAtomic64 volatile *a, isize time_out); // NOTE: time_out = -1 as default
    ZPL_DEF void zpl_atomic64_spin_unlock     (zplAtomic64 volatile *a);
    ZPL_DEF b32  zpl_atomic64_try_acquire_lock(zplAtomic64 volatile *a);
    
    
    ZPL_DEF void *zpl_atomic_ptr_load            (zplAtomicPtr const volatile *a);
    ZPL_DEF void  zpl_atomic_ptr_store           (zplAtomicPtr volatile *a, void *value);
    ZPL_DEF void *zpl_atomic_ptr_compare_exchange(zplAtomicPtr volatile *a, void *expected, void *desired);
    ZPL_DEF void *zpl_atomic_ptr_exchanged       (zplAtomicPtr volatile *a, void *desired);
    ZPL_DEF void *zpl_atomic_ptr_fetch_add       (zplAtomicPtr volatile *a, void *operand);
    ZPL_DEF void *zpl_atomic_ptr_fetch_and       (zplAtomicPtr volatile *a, void *operand);
    ZPL_DEF void *zpl_atomic_ptr_fetch_or        (zplAtomicPtr volatile *a, void *operand);
    ZPL_DEF b32   zpl_atomic_ptr_spin_lock       (zplAtomicPtr volatile *a, isize time_out); // NOTE: time_out = -1 as default
    ZPL_DEF void  zpl_atomic_ptr_spin_unlock     (zplAtomicPtr volatile *a);
    ZPL_DEF b32   zpl_atomic_ptr_try_acquire_lock(zplAtomicPtr volatile *a);
...
#if defined(ZPL_SYSTEM_WINDOWS)
    typedef struct zplSemaphore { void *win32_handle; }     zplSemaphore;
#elif defined(ZPL_SYSTEM_OSX)
    typedef struct zplSemaphore { semaphore_t osx_handle; } zplSemaphore;
#elif defined(ZPL_SYSTEM_UNIX)
    typedef struct zplSemaphore { sem_t unix_handle; }      zplSemaphore;
#else
#error
#endif
    
    ZPL_DEF void zpl_semaphore_init   (zplSemaphore *s);
    ZPL_DEF void zpl_semaphore_destroy(zplSemaphore *s);
    ZPL_DEF void zpl_semaphore_post   (zplSemaphore *s, i32 count);
    ZPL_DEF void zpl_semaphore_release(zplSemaphore *s); // NOTE: zpl_semaphore_post(s, 1)
    ZPL_DEF void zpl_semaphore_wait   (zplSemaphore *s);
    
    
    // Mutex
    // TODO: Should this be replaced with a CRITICAL_SECTION on win32 or is the better?
    typedef struct zplMutex {
        zplSemaphore semaphore;
        zplAtomic32  counter;
        zplAtomic32  owner;
        i32         recursion;
    } zplMutex;
    
    ZPL_DEF void zpl_mutex_init    (zplMutex *m);
    ZPL_DEF void zpl_mutex_destroy (zplMutex *m);
    ZPL_DEF void zpl_mutex_lock    (zplMutex *m);
    ZPL_DEF b32  zpl_mutex_try_lock(zplMutex *m);
    ZPL_DEF void zpl_mutex_unlock  (zplMutex *m);
    
#define ZPL_THREAD_PROC(name) void name(void *data)
    typedef ZPL_THREAD_PROC(zplThreadProc);
    
    typedef struct zplThread {
#if defined(ZPL_SYSTEM_WINDOWS)
        void *        win32_handle;
#else
    ...
    ZPL_DEF void zpl_thread_init            (zplThread *t);
    ZPL_DEF void zpl_thread_destory         (zplThread *t);
    ZPL_DEF void zpl_thread_start           (zplThread *t, zplThreadProc *proc, void *data);
    ZPL_DEF void zpl_thread_start_with_stack(zplThread *t, zplThreadProc *proc, void *data, isize stack_size);
    ZPL_DEF void zpl_thread_join            (zplThread *t);
    ZPL_DEF b32  zpl_thread_is_running      (zplThread const *t);
    ZPL_DEF u32  zpl_thread_current_id      (void);
    ZPL_DEF void zpl_thread_set_name        (zplThread *t, char const *name);
    
    
    // NOTE: Thread Merge Operation
    // Based on Sean Barrett's stb_sync
    typedef struct zplSync {
        ...
        zplMutex start;
        zplMutex mutex;
        zplSemaphore release;
    } zplSync;
    
    ZPL_DEF void zpl_sync_init          (zplSync *s);
    ZPL_DEF void zpl_sync_destroy       (zplSync *s);
    ZPL_DEF void zpl_sync_set_target    (zplSync *s, i32 count);
    ZPL_DEF void zpl_sync_release       (zplSync *s);
    ZPL_DEF i32  zpl_sync_reach         (zplSync *s);
    ZPL_DEF void zpl_sync_reach_and_wait(zplSync *s);
    
    
    
#if defined(ZPL_SYSTEM_WINDOWS)
    
    typedef struct zplAffinity {
        b32   is_accurate;
        isize core_count;
        isize thread_count;
...
#error TODO: Unknown system
    ...
    ZPL_DEF void  zpl_affinity_init   (zplAffinity *a);
    ZPL_DEF void  zpl_affinity_destroy(zplAffinity *a);
    ZPL_DEF b32   zpl_affinity_set    (zplAffinity *a, isize core, isize thread);
    ZPL_DEF isize zpl_affinity_thread_count_for_core(zplAffinity *a, isize core);
    ...
    typedef struct zplVirtualMemory {
        void *data;
        isize size;
    } zplVirtualMemory;
    
    ZPL_DEF zplVirtualMemory zpl_virtual_memory(void *data, isize size);
    ZPL_DEF zplVirtualMemory zpl_vm_alloc      (void *addr, isize size);
    ZPL_DEF b32             zpl_vm_free       (zplVirtualMemory vm);
    ZPL_DEF zplVirtualMemory zpl_vm_trim       (zplVirtualMemory vm, isize lead_size, isize size);
    ZPL_DEF b32             zpl_vm_purge      (zplVirtualMemory vm);
    ...
    typedef enum zplAllocationType {
        zplAllocation_Alloc,
        zplAllocation_Free,
        zplAllocation_FreeAll,
        zplAllocation_Resize,
    } zplAllocationType;
...
#define ZPL_ALLOCATOR_PROC(name)                         \
    ...
    u64 flags)
    typedef ZPL_ALLOCATOR_PROC(zplAllocatorProc);
    
    typedef struct zplAllocator {
        zplAllocatorProc *proc;
        void *           data;
    } zplAllocator;
    
    typedef enum zplAllocatorFlag {
        zplAllocatorFlag_ClearToZero = ZPL_BIT(0),
    } zplAllocatorFlag;
...
#define ZPL_DEFAULT_ALLOCATOR_FLAGS (zplAllocatorFlag_ClearToZero)
#endif
    
    ZPL_DEF void *zpl_alloc_align (zplAllocator a, isize size, isize alignment);
    ZPL_DEF void *zpl_alloc       (zplAllocator a, isize size);
    ZPL_DEF void  zpl_free        (zplAllocator a, void *ptr);
    ZPL_DEF void  zpl_free_all    (zplAllocator a);
    ZPL_DEF void *zpl_resize      (zplAllocator a, void *ptr, isize old_size, isize new_size);
    ZPL_DEF void *zpl_resize_align(zplAllocator a, void *ptr, isize old_size, isize new_size, isize alignment);
    
    ZPL_DEF void *zpl_alloc_copy      (zplAllocator a, void const *src, isize size);
    ZPL_DEF void *zpl_alloc_copy_align(zplAllocator a, void const *src, isize size, isize alignment);
    ZPL_DEF char *zpl_alloc_str       (zplAllocator a, char const *str);
    ZPL_DEF char *zpl_alloc_str_len   (zplAllocator a, char const *str, isize len);
...
#define zpl_alloc_array(allocator_, Type, count) (Type *)zpl_alloc(allocator_, zpl_size_of(Type) * (count))
    ...
    ZPL_DEF void *zpl_default_resize_align(zplAllocator a, void *ptr, isize old_size, isize new_size, isize alignment);
    
    ZPL_DEF zplAllocator zpl_heap_allocator(void);
...
#define zpl_mfree(ptr) zpl_free(zpl_heap_allocator(), ptr)
    ...
    // Arena Allocator
    //
    typedef struct zplArena {
        zplAllocator backing;
        void *      physical_start;
        isize       total_size;
    ...
    ZPL_DEF void zpl_arena_init_from_memory   (zplArena *arena, void *start, isize size);
    ZPL_DEF void zpl_arena_init_from_allocator(zplArena *arena, zplAllocator backing, isize size);
    ZPL_DEF void zpl_arena_init_sub           (zplArena *arena, zplArena *parent_arena, isize size);
    ZPL_DEF void zpl_arena_free               (zplArena *arena);
    
    ZPL_DEF isize zpl_arena_alignment_of  (zplArena *arena, isize alignment);
    ZPL_DEF isize zpl_arena_size_remaining(zplArena *arena, isize alignment);
    ZPL_DEF void  zpl_arena_check         (zplArena *arena);
    
    
    // Allocation Types: alloc, free_all, resize
    ZPL_DEF zplAllocator zpl_arena_allocator(zplArena *arena);
    ZPL_DEF ZPL_ALLOCATOR_PROC(zpl_arena_allocator_proc);
    
    
    
    typedef struct zplTempArenaMemory {
        zplArena *arena;
        isize    original_count;
    } zplTempArenaMemory;
    
    ZPL_DEF zplTempArenaMemory zpl_temp_arena_memory_begin(zplArena *arena);
    ZPL_DEF void              zpl_temp_arena_memory_end  (zplTempArenaMemory tmp_mem);
    ...
    ZPL_DEF void zpl_pool_init      (zplPool *pool, zplAllocator backing, isize num_blocks, isize block_size);
    ZPL_DEF void zpl_pool_init_align(zplPool *pool, zplAllocator backing, isize num_blocks, isize block_size, isize block_align);
    ZPL_DEF void zpl_pool_free      (zplPool *pool);
    
    // Allocation Types: alloc, free
    ZPL_DEF zplAllocator zpl_pool_allocator(zplPool *pool);
    ZPL_DEF ZPL_ALLOCATOR_PROC(zpl_pool_allocator_proc);
    
    typedef struct zplAllocationHeader {
        isize size;
    } zplAllocationHeader;
    
    ZPL_DEF zplAllocationHeader *zpl_allocation_header     (void *data);
    ZPL_DEF void                zpl_allocation_header_fill(zplAllocationHeader *header, void *data, isize size);
...
#error
    ...
    typedef struct zplScratchMemory {
        void *physical_start;
        isize total_size;
        void *alloc_point;
        void *free_point;
    ...
    ZPL_DEF void zpl_scratch_memory_init     (zplScratchMemory *s, void *start, isize size);
    ZPL_DEF b32  zpl_scratch_memory_is_in_use(zplScratchMemory *s, void *ptr);
    
    
    // Allocation Types: alloc, free, free_all, resize
    ZPL_DEF zplAllocator zpl_scratch_allocator(zplScratchMemory *s);
    ...
    typedef struct zplStackMemory {
        void *physical_start;
        isize total_size;
        void *current;
    } zplStackMemory;
    
    ZPL_DEF void zpl_stack_memory_init     (zplStackMemory *s, void *start, isize size);
    ZPL_DEF b32  zpl_stack_memory_is_in_use(zplStackMemory *s, void *ptr);

    
    // Allocation Types: alloc, free, free_all, resize
    ZPL_DEF zplAllocator zpl_stack_allocator(zplStackMemory *s);
...
#define ZPL_COMPARE_PROC(name) int name(void const *a, void const *b)
    typedef ZPL_COMPARE_PROC(zplCompareProc);
...
#define zpl_sort_array(array, count, compare_proc) zpl_sort(array, count, zpl_size_of(*(array)), compare_proc)
    ZPL_DEF void zpl_sort(void *base, isize count, isize size, zplCompareProc compare_proc);
...
#define zpl_binary_search_array(array, count, key, compare_proc) zpl_binary_search(array, count, zpl_size_of(*(array)), key, compare_proc)
    ZPL_DEF isize zpl_binary_search(void const *base, isize count, isize size, void const *key, zplCompareProc compare_proc);
...
#define zpl_reverse_array(array, count) zpl_reverse(array, count, zpl_size_of(*(array)))
    ...
    ////////////////////////////////////////////////////////////////
    //
    // zplString - C Read-Only-Compatible
    //
    //
    /*
    Reasoning:
...
#include "zpl.h"
    int main(int argc, char **argv) {
        zplString str = zpl_string_make("Hello");
        zplString other_str = zpl_string_make_length(", ", 2);
    ...
    typedef char *zplString;
    
    // NOTE: If you only need a small string, just use a standard c string or change the size from isize to u16, etc.
    typedef struct zplStringHeader {
        zplAllocator allocator;
        isize       length;
        isize       capacity;
    } zplStringHeader;
    
#define ZPL_STRING_HEADER(str) (cast(zplStringHeader *)(str) - 1)
    
    ZPL_DEF zplString zpl_string_make           (zplAllocator a, char const *str);
    ZPL_DEF zplString zpl_string_make_length    (zplAllocator a, void const *str, isize num_bytes);
    ZPL_DEF void     zpl_string_free           (zplString str);
    ZPL_DEF zplString zpl_string_duplicate      (zplAllocator a, zplString const str);
    ZPL_DEF isize    zpl_string_length         (zplString const str);
    ZPL_DEF isize    zpl_string_capacity       (zplString const str);
    ZPL_DEF isize    zpl_string_available_space(zplString const str);
    ZPL_DEF void     zpl_string_clear          (zplString str);
    ZPL_DEF zplString zpl_string_append         (zplString str, zplString const other);
    ZPL_DEF zplString zpl_string_append_length  (zplString str, void const *other, isize num_bytes);
    ZPL_DEF zplString zpl_string_appendc        (zplString str, char const *other);
    ZPL_DEF zplString zpl_string_set            (zplString str, char const *cstr);
    ZPL_DEF zplString zpl_string_make_space_for (zplString str, isize add_len);
    ZPL_DEF isize    zpl_string_allocation_size(zplString const str);
    ZPL_DEF b32      zpl_string_are_equal      (zplString const lhs, zplString const rhs);
    ZPL_DEF zplString zpl_string_trim           (zplString str, char const *cut_set);
    ZPL_DEF zplString zpl_string_trim_space     (zplString str); // Whitespace ` \t\r\n\v\f`
    ...
    //
    //
    // zplBuffer(Type) works like zplString or zplArray where the actual type is just a pointer to the first
    // element.
    //
    // Available Procedures for zplBuffer(Type)
    // zpl_buffer_init
    // zpl_buffer_free
    // zpl_buffer_append
    // zpl_buffer_appendv
    ...
    typedef struct zplBufferHeader {
        isize count;
        isize capacity;
    } zplBufferHeader;
    
#define zplBuffer(Type) Type *
    
#define ZPL_BUFFER_HEADER(x)   (cast(zplBufferHeader *)(x) - 1)
...
#define zpl_buffer_init(x, allocator, cap) do { \
        void **nx = cast(void **)&(x); \
        zplBufferHeader *zpl__bh = cast(zplBufferHeader *)zpl_alloc((allocator), (cap)*zpl_size_of(*(x))); \
        zpl__bh->count = 0; \
...
#define zpl_buffer_clear(x) do { zpl_buffer_count(x) = 0; } while (0)
    ...
    //
    // zplArray(Type) works like zplString or zplBuffer where the actual type is just a pointer to the first
    // element.
    //
    // Available Procedures for zplArray(Type)
    // zpl_array_init
    // zpl_array_free
    ...
    void foo(void) {
        ...
        zplAllocator a = zpl_heap_allocator();
        zplArray(int) items;
    ...
    typedef struct zplArrayHeader {
        zplAllocator allocator;
        isize       count;
        isize       capacity;
    } zplArrayHeader;
    
#define zplArray(Type) Type *
...
#define ZPL_ARRAY_HEADER(x)    (cast(zplArrayHeader *)(x) - 1)
...
#define zpl_array_init_reserve(x, allocator_, cap) do { \
        ...
        zplArrayHeader *zpl__ah = cast(zplArrayHeader *)zpl_alloc(allocator_, zpl_size_of(zplArrayHeader)+zpl_size_of(*(x))*(cap)); \
...
#define zpl_array_free(x) do { \
        zplArrayHeader *zpl__ah = ZPL_ARRAY_HEADER(x); \
...
#define zpl_array_set_capacity(x, capacity) do { \
    ...
    // TODO: Decide on a decent growing formula for zplArray
...
#define zpl_array_reserve(x, new_capacity) do { \
    ...
    typedef struct zplHashTableFindResult {
        isize hash_index;
        isize entry_prev;
        isize entry_index;
    } zplHashTableFindResult;
...
#define ZPL_TABLE_DECLARE(PREFIX, NAME, FUNC, VALUE) \
    ...
    \
    typedef struct NAME { \
        zplArray(isize) hashes; \
        zplArray(ZPL_JOIN2(NAME,Entry)) entries; \
    } NAME; \
    \
    PREFIX void                  ZPL_JOIN2(FUNC,init)       (NAME *h, zplAllocator a); \
...
#define ZPL_TABLE_DEFINE(NAME, FUNC, VALUE) \
    void ZPL_JOIN2(FUNC,init)(NAME *h, zplAllocator a) { \
    ...
    \
    zpl_internal zplHashTableFindResult ZPL_JOIN2(FUNC,_find)(NAME *h, u64 key) { \
        zplHashTableFindResult r = {-1, -1, -1}; \
    ...
    typedef u32 zplFileMode;
    typedef enum zplFileModeFlag {
        zplFileMode_Read       = ZPL_BIT(0),
        zplFileMode_Write      = ZPL_BIT(1),
        zplFileMode_Append     = ZPL_BIT(2),
        zplFileMode_Rw         = ZPL_BIT(3),
        
        zplFileMode_Modes = zplFileMode_Read | zplFileMode_Write | zplFileMode_Append | zplFileMode_Rw,
    } zplFileModeFlag;
    
    // NOTE: Only used internally and for the file operations
    typedef enum zplSeekWhenceType {
        zplSeekWhence_Begin   = 0,
        zplSeekWhence_Current = 1,
        zplSeekWhence_End     = 2,
    } zplSeekWhenceType;
    
    typedef enum zplFileError {
        zplFileError_None,
        zplFileError_Invalid,
        zplFileError_Exists,
        zplFileError_NotExists,
        zplFileError_Permission,
        zplFileError_TruncationFailure,
    } zplFileError;
    
    typedef union zplFileDescriptor {
        void *  p;
        intptr  i;
        uintptr u;
    } zplFileDescriptor;
    
    typedef struct zplFileOperations zplFileOperations;
    
#define ZPL_FILE_OPEN_PROC(name)     zplFileError name(zplFileDescriptor *fd, zplFileOperations *ops, zplFileMode mode, char const *filename)
#define ZPL_FILE_READ_AT_PROC(name)  b32         name(zplFileDescriptor fd, void *buffer, isize size, i64 offset, isize *bytes_read, b32 stop_at_newline)
#define ZPL_FILE_WRITE_AT_PROC(name) b32         name(zplFileDescriptor fd, void const *buffer, isize size, i64 offset, isize *bytes_written)
#define ZPL_FILE_SEEK_PROC(name)     b32         name(zplFileDescriptor fd, i64 offset, zplSeekWhenceType whence, i64 *new_offset)
#define ZPL_FILE_CLOSE_PROC(name)    void        name(zplFileDescriptor fd)
    typedef ZPL_FILE_OPEN_PROC(zplFileOpenProc);
    typedef ZPL_FILE_READ_AT_PROC(zplFileReadProc);
    typedef ZPL_FILE_WRITE_AT_PROC(zplFileWriteProc);
    typedef ZPL_FILE_SEEK_PROC(zplFileSeekProc);
    typedef ZPL_FILE_CLOSE_PROC(zplFileCloseProc);
    
    struct zplFileOperations {
        zplFileReadProc  *read_at;
        zplFileWriteProc *write_at;
        zplFileSeekProc  *seek;
        zplFileCloseProc *close;
    };
    
    extern zplFileOperations const zplDefaultFileOperations;
    
    
    // typedef struct zplDirInfo {
    // 	u8 *buf;
    // 	isize buf_count;
    // 	isize buf_pos;
    // } zplDirInfo;
    
    typedef u64 zplFileTime;
    
    typedef struct zplFile {
        zplFileOperations ops;
        zplFileDescriptor fd;
        char const *     filename;
        zplFileTime       last_write_time;
        // zplDirInfo *   dir_info; // TODO: Get directory info
    } zplFile;
    
    // TODO: zplAsyncFile
    
    typedef enum zplFileStandardType {
        zplFileStandard_Input,
        zplFileStandard_Output,
        zplFileStandard_Error,
        
        zplFileStandard_Count,
    } zplFileStandardType;
    
    ZPL_DEF zplFile *const zpl_file_get_standard(zplFileStandardType std);
    
    ZPL_DEF zplFileError zpl_file_create        (zplFile *file, char const *filename);
    ZPL_DEF zplFileError zpl_file_open          (zplFile *file, char const *filename);
    ZPL_DEF zplFileError zpl_file_open_mode     (zplFile *file, zplFileMode mode, char const *filename);
    ZPL_DEF zplFileError zpl_file_new           (zplFile *file, zplFileDescriptor fd, zplFileOperations ops, char const *filename);
    ZPL_DEF b32         zpl_file_read_at_check (zplFile *file, void *buffer, isize size, i64 offset, isize *bytes_read);
    ZPL_DEF b32         zpl_file_write_at_check(zplFile *file, void const *buffer, isize size, i64 offset, isize *bytes_written);
    ZPL_DEF b32         zpl_file_read_at       (zplFile *file, void *buffer, isize size, i64 offset);
    ZPL_DEF b32         zpl_file_write_at      (zplFile *file, void const *buffer, isize size, i64 offset);
    ZPL_DEF i64         zpl_file_seek          (zplFile *file, i64 offset);
    ZPL_DEF i64         zpl_file_seek_to_end   (zplFile *file);
    ZPL_DEF i64         zpl_file_skip          (zplFile *file, i64 bytes); // NOTE: Skips a certain amount of bytes
    ZPL_DEF i64         zpl_file_tell          (zplFile *file);
    ZPL_DEF zplFileError zpl_file_close         (zplFile *file);
    ZPL_DEF b32         zpl_file_read          (zplFile *file, void *buffer, isize size);
    ZPL_DEF b32         zpl_file_write         (zplFile *file, void const *buffer, isize size);
    ZPL_DEF i64         zpl_file_size          (zplFile *file);
    ZPL_DEF char const *zpl_file_name          (zplFile *file);
    ZPL_DEF zplFileError zpl_file_truncate      (zplFile *file, i64 size);
    ZPL_DEF b32         zpl_file_has_changed   (zplFile *file); // NOTE: Changed since lasted checked
    // TODO:
    // zplFileError zpl_file_temp(zplFile *file);
    //
    
    typedef struct zplFileContents {
        zplAllocator allocator;
        void *      data;
        isize       size;
    } zplFileContents;
    ...
    ZPL_DEF zplFileContents zpl_file_read_contents(zplAllocator a, b32 zero_terminate, char const *filepath);
    ZPL_DEF void           zpl_file_free_contents(zplFileContents *fc);
    
    
    // TODO: Should these have different na,es as they do not take in a zplFile * ???
    ZPL_DEF b32        zpl_file_exists         (char const *filepath);
    ZPL_DEF zplFileTime zpl_file_last_write_time(char const *filepath);
...
#define ZPL_PATH_SEPARATOR '/'
    ...
    ZPL_DEF char *      zpl_path_get_full_name(zplAllocator a, char const *path);
    ...
    ZPL_DEF isize zpl_fprintf       (zplFile *f, char const *fmt, ...) ZPL_PRINTF_ARGS(2);
    ZPL_DEF isize zpl_fprintf_va    (zplFile *f, char const *fmt, va_list va);
    ...
    typedef void *zplDllHandle;
    typedef void (*zplDllProc)(void);
    
    ZPL_DEF zplDllHandle zpl_dll_load        (char const *filepath);
    ZPL_DEF void        zpl_dll_unload      (zplDllHandle dll);
    ZPL_DEF zplDllProc   zpl_dll_proc_address(zplDllHandle dll, char const *proc_name);
    ...
    typedef struct zplRandom {
        u32 offsets[8];
        u32 value;
    } zplRandom;
    
    // NOTE: Generates from numerous sources to produce a decent pseudo-random seed
    ZPL_DEF void  zpl_random_init          (zplRandom *r);
    ZPL_DEF u32   zpl_random_gen_u32       (zplRandom *r);
    ZPL_DEF u32   zpl_random_gen_u32_unique(zplRandom *r);
    ZPL_DEF u64   zpl_random_gen_u64       (zplRandom *r); // NOTE: (zpl_random_gen_u32() << 32) | zpl_random_gen_u32()
    ZPL_DEF isize zpl_random_gen_isize     (zplRandom *r);
    ZPL_DEF i64   zpl_random_range_i64     (zplRandom *r, i64 lower_inc, i64 higher_inc);
    ZPL_DEF isize zpl_random_range_isize   (zplRandom *r, isize lower_inc, isize higher_inc);
    ZPL_DEF f64   zpl_random_range_f64     (zplRandom *r, f64 lower_inc, f64 higher_inc);
...
#define ZPL__HAS_ZERO(x) ((x)-ZPL__ONES & ~(x) & ZPL__HIGHS)
    ...
    zpl_inline void *zpl_alloc_align (zplAllocator a, isize size, isize alignment)                                { return a.proc(a.data, zplAllocation_Alloc, size, alignment, NULL, 0, ZPL_DEFAULT_ALLOCATOR_FLAGS); }
    zpl_inline void *zpl_alloc       (zplAllocator a, isize size)                                                 { return zpl_alloc_align(a, size, ZPL_DEFAULT_MEMORY_ALIGNMENT); }
    zpl_inline void  zpl_free        (zplAllocator a, void *ptr)                                                  { if (ptr != NULL) a.proc(a.data, zplAllocation_Free, 0, 0, ptr, 0, ZPL_DEFAULT_ALLOCATOR_FLAGS); }
    zpl_inline void  zpl_free_all    (zplAllocator a)                                                             { a.proc(a.data, zplAllocation_FreeAll, 0, 0, NULL, 0, ZPL_DEFAULT_ALLOCATOR_FLAGS); }
    zpl_inline void *zpl_resize      (zplAllocator a, void *ptr, isize old_size, isize new_size)                  { return zpl_resize_align(a, ptr, old_size, new_size, ZPL_DEFAULT_MEMORY_ALIGNMENT); }
    zpl_inline void *zpl_resize_align(zplAllocator a, void *ptr, isize old_size, isize new_size, isize alignment) { return a.proc(a.data, zplAllocation_Resize, new_size, alignment, ptr, old_size, ZPL_DEFAULT_ALLOCATOR_FLAGS); }
    
    zpl_inline void *zpl_alloc_copy      (zplAllocator a, void const *src, isize size)                  { return zpl_memcopy(zpl_alloc(a, size), src, size); }
    zpl_inline void *zpl_alloc_copy_align(zplAllocator a, void const *src, isize size, isize alignment) { return zpl_memcopy(zpl_alloc_align(a, size, alignment), src, size); }
    
    zpl_inline char *zpl_alloc_str(zplAllocator a, char const *str) {
        return zpl_alloc_str_len(a, str, zpl_strlen(str));
    }
    
    zpl_inline char *zpl_alloc_str_len(zplAllocator a, char const *str, isize len) {
        char *result;
    ...
    zpl_inline void *zpl_default_resize_align(zplAllocator a, void *old_memory, isize old_size, isize new_size, isize alignment) {
    ...
    zpl_inline i32  zpl_atomic32_load (zplAtomic32 const volatile *a)      { return a->value;  }
    zpl_inline void zpl_atomic32_store(zplAtomic32 volatile *a, i32 value) { a->value = value; }
    
    zpl_inline i32 zpl_atomic32_compare_exchange(zplAtomic32 volatile *a, i32 expected, i32 desired) {
        return _InterlockedCompareExchange(cast(long volatile *)a, desired, expected);
    }
    zpl_inline i32 zpl_atomic32_exchanged(zplAtomic32 volatile *a, i32 desired) {
        return _InterlockedExchange(cast(long volatile *)a, desired);
    }
    zpl_inline i32 zpl_atomic32_fetch_add(zplAtomic32 volatile *a, i32 operand) {
        return _InterlockedExchangeAdd(cast(long volatile *)a, operand);
    }
    zpl_inline i32 zpl_atomic32_fetch_and(zplAtomic32 volatile *a, i32 operand) {
        return _InterlockedAnd(cast(long volatile *)a, operand);
    }
    zpl_inline i32 zpl_atomic32_fetch_or(zplAtomic32 volatile *a, i32 operand) {
        return _InterlockedOr(cast(long volatile *)a, operand);
    }
    
    zpl_inline i64 zpl_atomic64_load(zplAtomic64 const volatile *a) {
...
#error TODO: atomics for this CPU
    ...
    zpl_inline void zpl_atomic64_store(zplAtomic64 volatile *a, i64 value) {
#if defined(ZPL_ARCH_64_BIT)
        a->value = value;
#elif ZPL_CPU_X86
...
#error TODO: atomics for this CPU
    ...
    zpl_inline i64 zpl_atomic64_compare_exchange(zplAtomic64 volatile *a, i64 expected, i64 desired) {
        return _InterlockedCompareExchange64(cast(i64 volatile *)a, desired, expected);
    }
    
    zpl_inline i64 zpl_atomic64_exchanged(zplAtomic64 volatile *a, i64 desired) {
#if defined(ZPL_ARCH_64_BIT)
...
#error TODO: atomics for this CPU
    ...
    zpl_inline i64 zpl_atomic64_fetch_add(zplAtomic64 volatile *a, i64 operand) {
#if defined(ZPL_ARCH_64_BIT)
...
#error TODO: atomics for this CPU
    ...
    zpl_inline i64 zpl_atomic64_fetch_and(zplAtomic64 volatile *a, i64 operand) {
#if defined(ZPL_ARCH_64_BIT)
...
#error TODO: atomics for this CPU
    ...
    zpl_inline i64 zpl_atomic64_fetch_or(zplAtomic64 volatile *a, i64 operand) {
#if defined(ZPL_ARCH_64_BIT)
...
#error TODO: Implement Atomics for this CPU
    ...
    zpl_inline b32 zpl_atomic32_spin_lock(zplAtomic32 volatile *a, isize time_out) {
    ...
    zpl_inline void zpl_atomic32_spin_unlock(zplAtomic32 volatile *a) {
        zpl_atomic32_store(a, 0);
        zpl_mfence();
    }
    
    zpl_inline b32 zpl_atomic64_spin_lock(zplAtomic64 volatile *a, isize time_out) {
    ...
    zpl_inline void zpl_atomic64_spin_unlock(zplAtomic64 volatile *a) {
        zpl_atomic64_store(a, 0);
        zpl_mfence();
    }
    
    zpl_inline b32 zpl_atomic32_try_acquire_lock(zplAtomic32 volatile *a) {
        i32 old_value;
        zpl_yield_thread();
        old_value = zpl_atomic32_compare_exchange(a, 1, 0);
        zpl_mfence();
    ...
    zpl_inline b32 zpl_atomic64_try_acquire_lock(zplAtomic64 volatile *a) {
        i64 old_value;
        zpl_yield_thread();
        old_value = zpl_atomic64_compare_exchange(a, 1, 0);
        zpl_mfence();
    ...
    zpl_inline void *zpl_atomic_ptr_load(zplAtomicPtr const volatile *a) {
        return cast(void *)cast(intptr)zpl_atomic32_load(cast(zplAtomic32 const volatile *)a);
    }
    zpl_inline void zpl_atomic_ptr_store(zplAtomicPtr volatile *a, void *value) {
        zpl_atomic32_store(cast(zplAtomic32 volatile *)a, cast(i32)cast(intptr)value);
    }
    zpl_inline void *zpl_atomic_ptr_compare_exchange(zplAtomicPtr volatile *a, void *expected, void *desired) {
        return cast(void *)cast(intptr)zpl_atomic32_compare_exchange(cast(zplAtomic32 volatile *)a, cast(i32)cast(intptr)expected, cast(i32)cast(intptr)desired);
    }
    zpl_inline void *zpl_atomic_ptr_exchanged(zplAtomicPtr volatile *a, void *desired) {
        return cast(void *)cast(intptr)zpl_atomic32_exchanged(cast(zplAtomic32 volatile *)a, cast(i32)cast(intptr)desired);
    }
    zpl_inline void *zpl_atomic_ptr_fetch_add(zplAtomicPtr volatile *a, void *operand) {
        return cast(void *)cast(intptr)zpl_atomic32_fetch_add(cast(zplAtomic32 volatile *)a, cast(i32)cast(intptr)operand);
    }
    zpl_inline void *zpl_atomic_ptr_fetch_and(zplAtomicPtr volatile *a, void *operand) {
        return cast(void *)cast(intptr)zpl_atomic32_fetch_and(cast(zplAtomic32 volatile *)a, cast(i32)cast(intptr)operand);
    }
    zpl_inline void *zpl_atomic_ptr_fetch_or(zplAtomicPtr volatile *a, void *operand) {
        return cast(void *)cast(intptr)zpl_atomic32_fetch_or(cast(zplAtomic32 volatile *)a, cast(i32)cast(intptr)operand);
    }
    zpl_inline b32 zpl_atomic_ptr_spin_lock(zplAtomicPtr volatile *a, isize time_out) {
        return zpl_atomic32_spin_lock(cast(zplAtomic32 volatile *)a, time_out);
    }
    zpl_inline void zpl_atomic_ptr_spin_unlock(zplAtomicPtr volatile *a) {
        zpl_atomic32_spin_unlock(cast(zplAtomic32 volatile *)a);
    }
    zpl_inline b32 zpl_atomic_ptr_try_acquire_lock(zplAtomicPtr volatile *a) {
        return zpl_atomic32_try_acquire_lock(cast(zplAtomic32 volatile *)a);
    }
    
#elif defined(ZPL_ARCH_64_BIT)
    
    zpl_inline void *zpl_atomic_ptr_load(zplAtomicPtr const volatile *a) {
        return cast(void *)cast(intptr)zpl_atomic64_load(cast(zplAtomic64 const volatile *)a);
    }
    zpl_inline void zpl_atomic_ptr_store(zplAtomicPtr volatile *a, void *value) {
        zpl_atomic64_store(cast(zplAtomic64 volatile *)a, cast(i64)cast(intptr)value);
    }
    zpl_inline void *zpl_atomic_ptr_compare_exchange(zplAtomicPtr volatile *a, void *expected, void *desired) {
        return cast(void *)cast(intptr)zpl_atomic64_compare_exchange(cast(zplAtomic64 volatile *)a, cast(i64)cast(intptr)expected, cast(i64)cast(intptr)desired);
    }
    zpl_inline void *zpl_atomic_ptr_exchanged(zplAtomicPtr volatile *a, void *desired) {
        return cast(void *)cast(intptr)zpl_atomic64_exchanged(cast(zplAtomic64 volatile *)a, cast(i64)cast(intptr)desired);
    }
    zpl_inline void *zpl_atomic_ptr_fetch_add(zplAtomicPtr volatile *a, void *operand) {
        return cast(void *)cast(intptr)zpl_atomic64_fetch_add(cast(zplAtomic64 volatile *)a, cast(i64)cast(intptr)operand);
    }
    zpl_inline void *zpl_atomic_ptr_fetch_and(zplAtomicPtr volatile *a, void *operand) {
        return cast(void *)cast(intptr)zpl_atomic64_fetch_and(cast(zplAtomic64 volatile *)a, cast(i64)cast(intptr)operand);
    }
    zpl_inline void *zpl_atomic_ptr_fetch_or(zplAtomicPtr volatile *a, void *operand) {
        return cast(void *)cast(intptr)zpl_atomic64_fetch_or(cast(zplAtomic64 volatile *)a, cast(i64)cast(intptr)operand);
    }
    zpl_inline b32 zpl_atomic_ptr_spin_lock(zplAtomicPtr volatile *a, isize time_out) {
        return zpl_atomic64_spin_lock(cast(zplAtomic64 volatile *)a, time_out);
    }
    zpl_inline void zpl_atomic_ptr_spin_unlock(zplAtomicPtr volatile *a) {
        zpl_atomic64_spin_unlock(cast(zplAtomic64 volatile *)a);
    }
    zpl_inline b32 zpl_atomic_ptr_try_acquire_lock(zplAtomicPtr volatile *a) {
        return zpl_atomic64_try_acquire_lock(cast(zplAtomic64 volatile *)a);
    }
#endif
...
#error Unknown architecture
    ...
    zpl_inline void zpl_semaphore_release(zplSemaphore *s) { zpl_semaphore_post(s, 1); }
    
#if defined(ZPL_SYSTEM_WINDOWS)
    zpl_inline void zpl_semaphore_init   (zplSemaphore *s)            { s->win32_handle = CreateSemaphoreA(NULL, 0, I32_MAX, NULL); }
    zpl_inline void zpl_semaphore_destroy(zplSemaphore *s)            { CloseHandle(s->win32_handle); }
    zpl_inline void zpl_semaphore_post   (zplSemaphore *s, i32 count) { ReleaseSemaphore(s->win32_handle, count, NULL); }
    zpl_inline void zpl_semaphore_wait   (zplSemaphore *s)            { WaitForSingleObject(s->win32_handle, INFINITE); }
    
#elif defined(ZPL_SYSTEM_OSX)
    zpl_inline void zpl_semaphore_init   (zplSemaphore *s)            { semaphore_create(mach_task_self(), &s->osx_handle, SYNC_POLICY_FIFO, 0); }
    zpl_inline void zpl_semaphore_destroy(zplSemaphore *s)            { semaphore_destroy(mach_task_self(), s->osx_handle); }
    zpl_inline void zpl_semaphore_post   (zplSemaphore *s, i32 count) { while (count --> 0) semaphore_signal(s->osx_handle); }
    zpl_inline void zpl_semaphore_wait   (zplSemaphore *s)            { semaphore_wait(s->osx_handle); }
    
#elif defined(ZPL_SYSTEM_UNIX)
    zpl_inline void zpl_semaphore_init   (zplSemaphore *s)            { sem_init(&s->unix_handle, 0, 0); }
    zpl_inline void zpl_semaphore_destroy(zplSemaphore *s)            { sem_destroy(&s->unix_handle); }
    zpl_inline void zpl_semaphore_post   (zplSemaphore *s, i32 count) { while (count --> 0) sem_post(&s->unix_handle); }
    zpl_inline void zpl_semaphore_wait   (zplSemaphore *s)            { int i; do { i = sem_wait(&s->unix_handle); } while (i == -1 && errno == EINTR); }
    
#else
#error
#endif
    
    zpl_inline void zpl_mutex_init(zplMutex *m) {
    ...
    zpl_inline void zpl_mutex_destroy(zplMutex *m) { zpl_semaphore_destroy(&m->semaphore); }
    
    zpl_inline void zpl_mutex_lock(zplMutex *m) {
    ...
    zpl_inline b32 zpl_mutex_try_lock(zplMutex *m) {
        i32 thread_id = cast(i32)zpl_thread_current_id();
    ...
    zpl_inline void zpl_mutex_unlock(zplMutex *m) {
        i32 recursion;
        i32 thread_id = cast(i32)zpl_thread_current_id();
    ...
    void zpl_thread_init(zplThread *t) {
        zpl_zero_item(t);
#if defined(ZPL_SYSTEM_WINDOWS)
    ...
    void zpl_thread_destory(zplThread *t) {
        if (t->is_running) zpl_thread_join(t);
        zpl_semaphore_destroy(&t->semaphore);
    }
    
    
    zpl_inline void zpl__thread_run(zplThread *t) {
        zpl_semaphore_release(&t->semaphore);
        t->proc(t->data);
    }
    
#if defined(ZPL_SYSTEM_WINDOWS)
    zpl_inline DWORD __stdcall zpl__thread_proc(void *arg) { zpl__thread_run(cast(zplThread *)arg); return 0; }
#else
    zpl_inline void *          zpl__thread_proc(void *arg) { zpl__thread_run(cast(zplThread *)arg); return NULL; }
#endif
    
    zpl_inline void zpl_thread_start(zplThread *t, zplThreadProc *proc, void *data) { zpl_thread_start_with_stack(t, proc, data, 0); }
    
    zpl_inline void zpl_thread_start_with_stack(zplThread *t, zplThreadProc *proc, void *data, isize stack_size) {
    ...
    zpl_inline void zpl_thread_join(zplThread *t) {
    ...
    zpl_inline b32 zpl_thread_is_running(zplThread const *t) { return t->is_running != 0; }
...
#error Unsupported architecture for zpl_thread_current_id()
    ...
    void zpl_thread_set_name(zplThread *t, char const *name) {
#if defined(ZPL_COMPILER_MSVC)
#pragma pack(push, 8)
        typedef struct {
            DWORD       type;
...
#pragma pack(pop)
    ...
    void zpl_sync_destroy(zplSync *s) {
        if (s->waiting)
    ...
    void zpl_sync_set_target(zplSync *s, i32 count) {
    ...
    void zpl_sync_release(zplSync *s) {
        if (s->waiting) {
    ...
    void zpl_sync_reach_and_wait(zplSync *s) {
        zpl_mutex_lock(&s->mutex);
        ZPL_ASSERT(s->current < s->target);
        s->current++;
        if (s->current == s->target) {
    ...
    zpl_inline zplAllocator zpl_heap_allocator(void) {
        zplAllocator a;
    ...
    ZPL_ALLOCATOR_PROC(zpl_heap_allocator_proc) {
        ...
        switch (type) {
#if defined(ZPL_COMPILER_MSVC)
            case zplAllocation_Alloc:
            ptr = _aligned_malloc(size, alignment);
            if (flags & zplAllocatorFlag_ClearToZero)
                zpl_zero_size(ptr, size);
            break;
            case zplAllocation_Free:
            _aligned_free(old_memory);
            break;
            case zplAllocation_Resize:
...
#if defined(ZPL_SYSTEM_WINDOWS)
    void zpl_affinity_init(zplAffinity *a) {
    ...
    void zpl_affinity_destroy(zplAffinity *a) {
        zpl_unused(a);
    }
    
    
    b32 zpl_affinity_set(zplAffinity *a, isize core, isize thread) {
    ...
    isize zpl_affinity_thread_count_for_core(zplAffinity *a, isize core) {
    ...
    b32 zpl_affinity_set(zplAffinity *a, isize core, isize thread_index) {
        isize index;
        thread_t thread;
...
#elif defined(ZPL_SYSTEM_LINUX)
    // IMPORTANT TODO: This zplAffinity stuff for linux needs be improved a lot!
...
#error TODO: Unknown system
    ...
    zplVirtualMemory zpl_virtual_memory(void *data, isize size) {
        zplVirtualMemory vm;
        vm.data = data;
        vm.size = size;
        return vm;
...
#if defined(ZPL_SYSTEM_WINDOWS)
    zpl_inline zplVirtualMemory zpl_vm_alloc(void *addr, isize size) {
        zplVirtualMemory vm;
        ZPL_ASSERT(size > 0);
    ...
    zpl_inline b32 zpl_vm_free(zplVirtualMemory vm) {
    ...
    zpl_inline zplVirtualMemory zpl_vm_trim(zplVirtualMemory vm, isize lead_size, isize size) {
        zplVirtualMemory new_vm = {0};
        void *ptr;
    ...
    zpl_inline b32 zpl_vm_purge(zplVirtualMemory vm) {
...
#define MAP_ANONYMOUS MAP_ANON
    ...
    zpl_inline void zpl_arena_init_from_memory(zplArena *arena, void *start, isize size) {
    ...
    zpl_inline void zpl_arena_init_from_allocator(zplArena *arena, zplAllocator backing, isize size) {
    ...
    zpl_inline void zpl_arena_init_sub(zplArena *arena, zplArena *parent_arena, isize size) { zpl_arena_init_from_allocator(arena, zpl_arena_allocator(parent_arena), size); }
    
    
    zpl_inline void zpl_arena_free(zplArena *arena) {
    ...
    zpl_inline isize zpl_arena_alignment_of(zplArena *arena, isize alignment) {
    ...
    zpl_inline isize zpl_arena_size_remaining(zplArena *arena, isize alignment) {
        isize result = arena->total_size - (arena->total_allocated + zpl_arena_alignment_of(arena, alignment));
        return result;
    }
    
    zpl_inline void zpl_arena_check(zplArena *arena) { ZPL_ASSERT(arena->temp_count == 0); }
    ...
    zpl_inline zplAllocator zpl_arena_allocator(zplArena *arena) {
        zplAllocator allocator;
    ...
    ZPL_ALLOCATOR_PROC(zpl_arena_allocator_proc) {
        zplArena *arena = cast(zplArena *)allocator_data;
        void *ptr = NULL;
        ...
        switch (type) {
            ...
            case zplAllocation_Resize: {
                ...
                zplAllocator a = zpl_arena_allocator(arena);
                ptr = zpl_default_resize_align(a, old_memory, old_size, size, alignment);
            } break;
        }
        return ptr;
    ...
    zpl_inline zplTempArenaMemory zpl_temp_arena_memory_begin(zplArena *arena) {
        zplTempArenaMemory tmp;
        tmp.arena = arena;
    ...
    zpl_inline void zpl_temp_arena_memory_end(zplTempArenaMemory tmp) {
    ...
    zpl_inline void zpl_pool_init(zplPool *pool, zplAllocator backing, isize num_blocks, isize block_size) {
        zpl_pool_init_align(pool, backing, num_blocks, block_size, ZPL_DEFAULT_MEMORY_ALIGNMENT);
    }
    
    void zpl_pool_init_align(zplPool *pool, zplAllocator backing, isize num_blocks, isize block_size, isize block_align) {
    ...
    zpl_inline void zpl_pool_free(zplPool *pool) {
    ...
    zpl_inline zplAllocator zpl_pool_allocator(zplPool *pool) {
        zplAllocator allocator;
    ...
    ZPL_ALLOCATOR_PROC(zpl_pool_allocator_proc) {
        zplPool *pool = cast(zplPool *)allocator_data;
        void *ptr = NULL;
    ...
    zpl_inline zplAllocationHeader *zpl_allocation_header(void *data) {
        isize *p = cast(isize *)data;
        while (p[-1] == cast(isize)(-1))
            p--;
        return cast(zplAllocationHeader *)p - 1;
    }
    
    zpl_inline void zpl_allocation_header_fill(zplAllocationHeader *header, void *data, isize size) {
        isize *ptr;
        header->size = size;
    ...
    void zpl_scratch_memory_init(zplScratchMemory *s, void *start, isize size) {
    ...
    b32 zpl_scratch_memory_is_in_use(zplScratchMemory *s, void *ptr) {
    ...
    zplAllocator zpl_scratch_allocator(zplScratchMemory *s) {
        zplAllocator a;
    ...
    ZPL_ALLOCATOR_PROC(zpl_scratch_allocator_proc) {
        zplScratchMemory *s = cast(zplScratchMemory *)allocator_data;
        void *ptr = NULL;
        ...
        switch (type) {
            case zplAllocation_Alloc: {
                ...
                zplAllocationHeader *header = cast(zplAllocationHeader *)pt;
                ...
                if (pt > end) {
                    ...
                    pt = s->physical_start;
                    header = cast(zplAllocationHeader *)pt;
            ...
            case zplAllocation_Free: {
                if (old_memory) {
                    ...
                    if (old_memory < s->physical_start || old_memory >= end) {
                        ...
                        // NOTE: Mark as free
                        zplAllocationHeader *h = zpl_allocation_header(old_memory);
                        ZPL_ASSERT((h->size & ZPL_ISIZE_HIGH_BIT) == 0);
                        h->size = h->size | ZPL_ISIZE_HIGH_BIT;
                        
                        while (s->free_point != s->alloc_point) {
                            zplAllocationHeader *header = cast(zplAllocationHeader *)s->free_point;
...
#define ZPL_STACK_ALLOC_OFFSET sizeof(u64)
    ...
    zpl_inline void zpl_stack_memory_init(zplStackMemory *s, void *start, isize size) {
        s->physical_start = start;
        s->total_size = size;
        s->current = s->physical_start;
    }
    
    zpl_inline b32 zpl_stack_memory_is_in_use(zplStackMemory *s, void *ptr) {
    ...
    typedef union zplStackMemoryHeader {
        void *a_ptr;
        u8   *a_u8;
        u64  *a_u64;
    } zplStackMemoryHeader;
    
    zpl_inline zplAllocator zpl_stack_allocator(zplStackMemory *s) {
        zplAllocator a;
    ...
    ZPL_ALLOCATOR_PROC(zpl_stack_allocator_proc) {
        zplStackMemory *s = cast(zplStackMemory *)allocator_data;
        void *ptr = NULL;
        ZPL_ASSERT_NOT_NULL(s);
...
#define ZPL__SORT_POP(_base, _limit) do { \
    ...
    void zpl_sort(void *base_, isize count, isize size, zplCompareProc cmp) {
        u8 *i, *j;
...
#define ZPL_RADIX_SORT_PROC_GEN(Type) ZPL_RADIX_SORT_PROC(Type) { \
    ...
    zpl_inline isize zpl_binary_search(void const *base, isize count, isize size, void const *key, zplCompareProc compare_proc) {
        isize start = 0;
        isize end = count;
    ...
    void zpl_shuffle(void *base, isize count, isize size) {
        u8 *a;
        isize i, j;
        zplRandom random; zpl_random_init(&random);
    ...
    zpl_inline void zpl__set_string_length  (zplString str, isize len) { ZPL_STRING_HEADER(str)->length = len; }
    zpl_inline void zpl__set_string_capacity(zplString str, isize cap) { ZPL_STRING_HEADER(str)->capacity = cap; }
    
    
    zpl_inline zplString zpl_string_make(zplAllocator a, char const *str) {
        isize len = str ? zpl_strlen(str) : 0;
        return zpl_string_make_length(a, str, len);
    }
    
    zplString zpl_string_make_length(zplAllocator a, void const *init_str, isize num_bytes) {
        isize header_size = zpl_size_of(zplStringHeader);
        void *ptr = zpl_alloc(a, header_size + num_bytes + 1);
        
        zplString str;
        zplStringHeader *header;
    ...
    zpl_inline void zpl_string_free(zplString str) {
        if (str) {
            zplStringHeader *header = ZPL_STRING_HEADER(str);
            zpl_free(header->allocator, header);
        }
        
    }
    
    zpl_inline zplString zpl_string_duplicate(zplAllocator a, zplString const str) { return zpl_string_make_length(a, str, zpl_string_length(str)); }
    
    zpl_inline isize zpl_string_length  (zplString const str) { return ZPL_STRING_HEADER(str)->length; }
    zpl_inline isize zpl_string_capacity(zplString const str) { return ZPL_STRING_HEADER(str)->capacity; }
    
    zpl_inline isize zpl_string_available_space(zplString const str) {
        zplStringHeader *h = ZPL_STRING_HEADER(str);
    ...
    zpl_inline void zpl_string_clear(zplString str) { zpl__set_string_length(str, 0); str[0] = '\0'; }
    
    zpl_inline zplString zpl_string_append(zplString str, zplString const other) { return zpl_string_append_length(str, other, zpl_string_length(other)); }
    
    zplString zpl_string_append_length(zplString str, void const *other, isize other_len) {
        if (other_len > 0) {
    ...
    zpl_inline zplString zpl_string_appendc(zplString str, char const *other) {
        return zpl_string_append_length(str, other, zpl_strlen(other));
    }
    
    
    zplString zpl_string_set(zplString str, char const *cstr) {
    ...
    zplString zpl_string_make_space_for(zplString str, isize add_len) {
        ...
        if (available >= add_len) {
            ...
            void *ptr, *new_ptr;
            zplAllocator a = ZPL_STRING_HEADER(str)->allocator;
            zplStringHeader *header;
            
            new_len = zpl_string_length(str) + add_len;
            ptr = ZPL_STRING_HEADER(str);
            old_size = zpl_size_of(zplStringHeader) + zpl_string_length(str) + 1;
            new_size = zpl_size_of(zplStringHeader) + new_len + 1;
            
            new_ptr = zpl_resize(a, ptr, old_size, new_size);
            if (new_ptr == NULL) return NULL;
            
            header = cast(zplStringHeader *)new_ptr;
            header->allocator = a;
            
            str = cast(zplString)(header+1);
    ...
    zpl_inline isize zpl_string_allocation_size(zplString const str) {
        isize cap = zpl_string_capacity(str);
        return zpl_size_of(zplStringHeader) + cap;
    }
    
    
    zpl_inline b32 zpl_string_are_equal(zplString const lhs, zplString const rhs) {
    ...
    zplString zpl_string_trim(zplString str, char const *cut_set) {
    ...
    zpl_inline zplString zpl_string_trim_space(zplString str) { return zpl_string_trim(str, " \t\r\n\v\f"); }
    ...
    typedef struct zplUtf8AcceptRange {
        u8 lo, hi;
    } zplUtf8AcceptRange;
    
    zpl_global zplUtf8AcceptRange const zpl__utf8_accept_ranges[] = {
        {0x80, 0xbf},
        {0xa0, 0xbf},
        {0x80, 0x9f},
        {0x90, 0xbf},
    ...
    zpl_no_inline void *zpl__array_set_capacity(void *array, isize capacity, isize element_size) {
        zplArrayHeader *h = ZPL_ARRAY_HEADER(array);
        ...
        {
            isize size = zpl_size_of(zplArrayHeader) + element_size*capacity;
            zplArrayHeader *nh = cast(zplArrayHeader *)zpl_alloc(h->allocator, size);
            zpl_memmove(nh, h, zpl_size_of(zplArrayHeader) + element_size*h->count);
    ...
    zpl_internal ZPL_FILE_READ_AT_PROC(zpl__win32_file_read) {
        b32 result = false;
        zpl__win32_file_seek(fd, offset, zplSeekWhence_Begin, NULL);
    ...
    zplFileOperations const zplDefaultFileOperations = {
    ...
    zpl_no_inline ZPL_FILE_OPEN_PROC(zpl__win32_file_open) {
        ...
        switch (mode & zplFileMode_Modes) {
            case zplFileMode_Read:
            desired_access = GENERIC_READ;
            creation_disposition = OPEN_EXISTING;
            break;
            case zplFileMode_Write:
            desired_access = GENERIC_WRITE;
            creation_disposition = CREATE_ALWAYS;
            break;
            case zplFileMode_Append:
            desired_access = GENERIC_WRITE;
            creation_disposition = OPEN_ALWAYS;
            break;
            case zplFileMode_Read | zplFileMode_Rw:
            desired_access = GENERIC_READ | GENERIC_WRITE;
            creation_disposition = OPEN_EXISTING;
            break;
            case zplFileMode_Write | zplFileMode_Rw:
            desired_access = GENERIC_READ | GENERIC_WRITE;
            creation_disposition = CREATE_ALWAYS;
            break;
            case zplFileMode_Append | zplFileMode_Rw:
            desired_access = GENERIC_READ | GENERIC_WRITE;
            creation_disposition = OPEN_ALWAYS;
            break;
            default:
            ZPL_PANIC("Invalid file mode");
            return zplFileError_Invalid;
        }
        ...
        if (handle == INVALID_HANDLE_VALUE) {
            ...
            switch (err) {
                case ERROR_FILE_NOT_FOUND: return zplFileError_NotExists;
                case ERROR_FILE_EXISTS:    return zplFileError_Exists;
                case ERROR_ALREADY_EXISTS: return zplFileError_Exists;
                case ERROR_ACCESS_DENIED:  return zplFileError_Permission;
            }
            return zplFileError_Invalid;
        }
        
        if (mode & zplFileMode_Append) {
            LARGE_INTEGER offset = {0};
            if (!SetFilePointerEx(handle, offset, NULL, zplSeekWhence_End)) {
                CloseHandle(handle);
                return zplFileError_Invalid;
            }
        }
        ...
        fd->p = handle;
        *ops = zplDefaultFileOperations;
        return zplFileError_None;
    }
    ...
    zpl_internal ZPL_FILE_WRITE_AT_PROC(zpl__posix_file_write) {
        isize res;
        i64 curr_offset = 0;
        zpl__posix_file_seek(fd, 0, zplSeekWhence_Current, &curr_offset);
    ...
    zplFileError zpl_file_new(zplFile *f, zplFileDescriptor fd, zplFileOperations ops, char const *filename) {
        zplFileError err = zplFileError_None;
    ...
    zplFileError zpl_file_open_mode(zplFile *f, zplFileMode mode, char const *filename) {
        zplFileError err;
#if defined(ZPL_SYSTEM_WINDOWS)
...
#endif
        if (err == zplFileError_None)
            return zpl_file_new(f, f->fd, f->ops, filename);
        return err;
    }
    
    zplFileError zpl_file_close(zplFile *f) {
        if (!f)
            return zplFileError_Invalid;
        ...
        if (!f->ops.read_at) f->ops = zplDefaultFileOperations;
        f->ops.close(f->fd);
        
        return zplFileError_None;
    }
    
    zpl_inline b32 zpl_file_read_at_check(zplFile *f, void *buffer, isize size, i64 offset, isize *bytes_read) {
        if (!f->ops.read_at) f->ops = zplDefaultFileOperations;
        return f->ops.read_at(f->fd, buffer, size, offset, bytes_read, false);
    }
    
    zpl_inline b32 zpl_file_write_at_check(zplFile *f, void const *buffer, isize size, i64 offset, isize *bytes_written) {
        if (!f->ops.read_at) f->ops = zplDefaultFileOperations;
        return f->ops.write_at(f->fd, buffer, size, offset, bytes_written);
    }
    
    
    zpl_inline b32 zpl_file_read_at(zplFile *f, void *buffer, isize size, i64 offset) {
        return zpl_file_read_at_check(f, buffer, size, offset, NULL);
    }
    
    zpl_inline b32 zpl_file_write_at(zplFile *f, void const *buffer, isize size, i64 offset) {
        return zpl_file_write_at_check(f, buffer, size, offset, NULL);
    }
    
    zpl_inline i64 zpl_file_seek(zplFile *f, i64 offset) {
        i64 new_offset = 0;
        if (!f->ops.read_at) f->ops = zplDefaultFileOperations;
        f->ops.seek(f->fd, offset, zplSeekWhence_Begin, &new_offset);
        return new_offset;
    }
    
    zpl_inline i64 zpl_file_seek_to_end(zplFile *f) {
        i64 new_offset = 0;
        if (!f->ops.read_at) f->ops = zplDefaultFileOperations;
        f->ops.seek(f->fd, 0, zplSeekWhence_End, &new_offset);
        return new_offset;
    }
    
    // NOTE: Skips a certain amount of bytes
    zpl_inline i64 zpl_file_skip(zplFile *f, i64 bytes) {
        i64 new_offset = 0;
        if (!f->ops.read_at) f->ops = zplDefaultFileOperations;
        f->ops.seek(f->fd, bytes, zplSeekWhence_Current, &new_offset);
        return new_offset;
    }
    
    zpl_inline i64 zpl_file_tell(zplFile *f) {
        i64 new_offset = 0;
        if (!f->ops.read_at) f->ops = zplDefaultFileOperations;
        f->ops.seek(f->fd, 0, zplSeekWhence_Current, &new_offset);
        return new_offset;
    }
    zpl_inline b32 zpl_file_read       (zplFile *f, void *buffer, isize size)       { return zpl_file_read_at(f, buffer, size, zpl_file_tell(f)); }
    zpl_inline b32 zpl_file_write      (zplFile *f, void const *buffer, isize size) { return zpl_file_write_at(f, buffer, size, zpl_file_tell(f)); }
    
    
    zplFileError zpl_file_create(zplFile *f, char const *filename) {
        return zpl_file_open_mode(f, zplFileMode_Write|zplFileMode_Rw, filename);
    }
    
    
    zplFileError zpl_file_open(zplFile *f, char const *filename) {
        return zpl_file_open_mode(f, zplFileMode_Read, filename);
    }
    
    
    char const *zpl_file_name(zplFile *f) { return f->filename ? f->filename : ""; }
    
    zpl_inline b32 zpl_file_has_changed(zplFile *f) {
        b32 result = false;
        zplFileTime last_write_time = zpl_file_last_write_time(f->filename);
    ...
    zpl_global zplFile zpl__std_files[zplFileStandard_Count] = {{0}};
    
    
#if defined(ZPL_SYSTEM_WINDOWS)
    
    zpl_inline zplFile *const zpl_file_get_standard(zplFileStandardType std) {
        if (!zpl__std_file_set) {
#define ZPL__SET_STD_FILE(type, v) zpl__std_files[type].fd.p = v; zpl__std_files[type].ops = zplDefaultFileOperations
            ZPL__SET_STD_FILE(zplFileStandard_Input,  GetStdHandle(STD_INPUT_HANDLE));
            ZPL__SET_STD_FILE(zplFileStandard_Output, GetStdHandle(STD_OUTPUT_HANDLE));
            ZPL__SET_STD_FILE(zplFileStandard_Error,  GetStdHandle(STD_ERROR_HANDLE));
#undef ZPL__SET_STD_FILE
    ...
    zpl_inline i64 zpl_file_size(zplFile *f) {
        LARGE_INTEGER size;
        GetFileSizeEx(f->fd.p, &size);
        return size.QuadPart;
    }
    
    zplFileError zpl_file_truncate(zplFile *f, i64 size) {
        zplFileError err = zplFileError_None;
        i64 prev_offset = zpl_file_tell(f);
        zpl_file_seek(f, size);
        if (!SetEndOfFile(f))
            err = zplFileError_TruncationFailure;
    ...
    zpl_inline zplFile *const zpl_file_get_standard(zplFileStandardType std) {
        if (!zpl__std_file_set) {
#define ZPL__SET_STD_FILE(type, v) zpl__std_files[type].fd.i = v; zpl__std_files[type].ops = zplDefaultFileOperations
            ZPL__SET_STD_FILE(zplFileStandard_Input,  0);
            ZPL__SET_STD_FILE(zplFileStandard_Output, 1);
            ZPL__SET_STD_FILE(zplFileStandard_Error,  2);
#undef ZPL__SET_STD_FILE
            zpl__std_file_set = true;
        }
        return &zpl__std_files[std];
    ...
    zpl_inline zplFileError zpl_file_truncate(zplFile *f, i64 size) {
        zplFileError err = zplFileError_None;
        int i = ftruncate(f->fd.i, size);
        if (i != 0) err = zplFileError_TruncationFailure;
        return err;
    }
...
#if defined(ZPL_SYSTEM_WINDOWS)
    zplFileTime zpl_file_last_write_time(char const *filepath) {
        u16 path[1024] = {0};
        ULARGE_INTEGER li = {0};
        ...
        return cast(zplFileTime)li.QuadPart;
    }
    ...
    zplFileTime zpl_file_last_write_time(char const *filepath) {
        ...
        return cast(zplFileTime)result;
    }
    ...
    zplFileContents zpl_file_read_contents(zplAllocator a, b32 zero_terminate, char const *filepath) {
        zplFileContents result = {0};
        zplFile file = {0};
        
        result.allocator = a;
        
        if (zpl_file_open(&file, filepath) == zplFileError_None) {
    ...
    void zpl_file_free_contents(zplFileContents *fc) {
    ...
    char *zpl_path_get_full_name(zplAllocator a, char const *path) {
#if defined(ZPL_SYSTEM_WINDOWS)
        // TODO: Make UTF-8
        char buf[300];
    ...
    isize zpl_fprintf(struct zplFile *f, char const *fmt, ...) {
        isize res;
        va_list va;
        va_start(va, fmt);
    ...
    zpl_inline isize zpl_printf_va(char const *fmt, va_list va) {
        return zpl_fprintf_va(zpl_file_get_standard(zplFileStandard_Output), fmt, va);
    }
    
    zpl_inline isize zpl_printf_err_va(char const *fmt, va_list va) {
        return zpl_fprintf_va(zpl_file_get_standard(zplFileStandard_Error), fmt, va);
    }
    
    zpl_inline isize zpl_fprintf_va(struct zplFile *f, char const *fmt, va_list va) {
    ...
    enum {
        zplFmt_Minus     = ZPL_BIT(0),
        zplFmt_Plus      = ZPL_BIT(1),
        zplFmt_Alt       = ZPL_BIT(2),
        zplFmt_Space     = ZPL_BIT(3),
        zplFmt_Zero      = ZPL_BIT(4),
        
        zplFmt_Char      = ZPL_BIT(5),
        zplFmt_Short     = ZPL_BIT(6),
        zplFmt_Int       = ZPL_BIT(7),
        zplFmt_Long      = ZPL_BIT(8),
        zplFmt_Llong     = ZPL_BIT(9),
        zplFmt_Size      = ZPL_BIT(10),
        zplFmt_Intptr    = ZPL_BIT(11),
        
        zplFmt_Unsigned  = ZPL_BIT(12),
        zplFmt_Lower     = ZPL_BIT(13),
        zplFmt_Upper     = ZPL_BIT(14),
        
        
        zplFmt_Done      = ZPL_BIT(30),
        
        zplFmt_Ints = zplFmt_Char|zplFmt_Short|zplFmt_Int|zplFmt_Long|zplFmt_Llong|zplFmt_Size|zplFmt_Intptr
    };
    ...
    zpl_internal isize zpl__print_string(char *text, isize max_len, zplprivFmtInfo *info, char const *str) {
        ...
        if (info && (info->width == 0 || info->flags & zplFmt_Minus)) {
            if (info->precision > 0)
            ...
            if (info->width > res) {
                ...
                char pad = (info->flags & zplFmt_Zero) ? '0' : ' ';
        ...
        if (info) {
            if (info->flags & zplFmt_Upper)
                zpl_str_to_upper(text);
            else if (info->flags & zplFmt_Lower)
    ...
    zpl_internal isize zpl__print_f64(char *text, isize max_len, zplprivFmtInfo *info, f64 arg) {
        ...
        if (arg) {
            ...
            if (arg < 0) {
                ...
                text++;
                arg = -arg;
            } else if (info->flags & zplFmt_Minus) {
                if (remaining > 1)
                    *text = '+', remaining--;
                text++;
            }
            ...
            if ((info->flags & zplFmt_Alt) || info->precision > 0) {
                i64 mult = 10;
                if (remaining > 1)
                    *text = '.', remaining--;
                text++;
        ...
        if (width > 0) {
            char fill = (info->flags & zplFmt_Zero) ? '0' : ' ';
    ...
    zpl_no_inline isize zpl_snprintf_va(char *text, isize max_len, char const *fmt, va_list va) {
        ...
        while (*fmt) {
            ...
            if (*fmt == '%') {
                do {
                    switch (*++fmt) {
                        case '-': info.flags |= zplFmt_Minus; break;
                        case '+': info.flags |= zplFmt_Plus;  break;
                        case '#': info.flags |= zplFmt_Alt;   break;
                        case ' ': info.flags |= zplFmt_Space; break;
                        case '0': info.flags |= zplFmt_Zero;  break;
                        default:  info.flags |= zplFmt_Done;  break;
                    }
                } while (!(info.flags & zplFmt_Done));
            }
            ...
            switch (*fmt++) {
                case 'h':
                if (*fmt == 'h') { // hh => char
                    info.flags |= zplFmt_Char;
                    fmt++;
                } else { // h => short
                    info.flags |= zplFmt_Short;
                }
                break;
                
                case 'l':
                if (*fmt == 'l') { // ll => long long
                    info.flags |= zplFmt_Llong;
                    fmt++;
                } else { // l => long
                    info.flags |= zplFmt_Long;
                }
                break;
                ...
                case 'z': // NOTE: usize
                info.flags |= zplFmt_Unsigned;
                // fallthrough
                case 't': // NOTE: isize
                info.flags |= zplFmt_Size;
                break;
            ...
            switch (*fmt) {
                ...
                case 'x':
                info.base = 16;
                info.flags |= (zplFmt_Unsigned | zplFmt_Lower);
                break;
                
                case 'X':
                info.base = 16;
                info.flags |= (zplFmt_Unsigned | zplFmt_Upper);
                break;
                ...
                case 'p':
                info.base = 16;
                info.flags |= (zplFmt_Lower|zplFmt_Unsigned|zplFmt_Alt|zplFmt_Intptr);
                break;
            ...
            if (info.base != 0) {
                if (info.flags & zplFmt_Unsigned) {
                    u64 value = 0;
                    switch (info.flags & zplFmt_Ints) {
                        case zplFmt_Char:   value = cast(u64)cast(u8) va_arg(va, int);       break;
                        case zplFmt_Short:  value = cast(u64)cast(u16)va_arg(va, int);       break;
                        case zplFmt_Long:   value = cast(u64)va_arg(va, unsigned long);      break;
                        case zplFmt_Llong:  value = cast(u64)va_arg(va, unsigned long long); break;
                        case zplFmt_Size:   value = cast(u64)va_arg(va, usize);              break;
                        case zplFmt_Intptr: value = cast(u64)va_arg(va, uintptr);            break;
                    ...
                    i64 value = 0;
                    switch (info.flags & zplFmt_Ints) {
                        case zplFmt_Char:   value = cast(i64)cast(i8) va_arg(va, int); break;
                        case zplFmt_Short:  value = cast(i64)cast(i16)va_arg(va, int); break;
                        case zplFmt_Long:   value = cast(i64)va_arg(va, long);         break;
                        case zplFmt_Llong:  value = cast(i64)va_arg(va, long long);    break;
                        case zplFmt_Size:   value = cast(i64)va_arg(va, usize);        break;
                        case zplFmt_Intptr: value = cast(i64)va_arg(va, uintptr);      break;
    ...
    zplDllHandle zpl_dll_load(char const *filepath) {
        return cast(zplDllHandle)LoadLibraryA(filepath);
    }
    zpl_inline void      zpl_dll_unload      (zplDllHandle dll)                        { FreeLibrary(cast(HMODULE)dll); }
    zpl_inline zplDllProc zpl_dll_proc_address(zplDllHandle dll, char const *proc_name) { return cast(zplDllProc)GetProcAddress(cast(HMODULE)dll, proc_name); }
    
#else // POSIX
    
    zplDllHandle zpl_dll_load(char const *filepath) {
        // TODO: Should this be RTLD_LOCAL?
        return cast(zplDllHandle)dlopen(filepath, RTLD_LAZY|RTLD_GLOBAL);
    }
    
    zpl_inline void      zpl_dll_unload      (zplDllHandle dll)                        { dlclose(dll); }
    zpl_inline zplDllProc zpl_dll_proc_address(zplDllHandle dll, char const *proc_name) { return cast(zplDllProc)dlsym(dll, proc_name); }
    ...
    zpl_global zplAtomic32 zpl__random_shared_counter = {0};
    ...
    void zpl_random_init(zplRandom *r) {
        u64 time, tick;
        isize i, j;
        u32 x = 0;
        r->value = 0;
    ...
    u32 zpl_random_gen_u32(zplRandom *r) {
        u32 x = r->value;
        u32 carry = 1;
        isize i;
        for (i = 0; i < zpl_count_of(r->offsets); i++) {
    ...
    u32 zpl_random_gen_u32_unique(zplRandom *r) {
        u32 x = r->value;
        isize i;
        r->value++;
        for (i = 0; i < zpl_count_of(r->offsets); i++) {
    ...
    u64 zpl_random_gen_u64(zplRandom *r) {
        return ((cast(u64)zpl_random_gen_u32(r)) << 32) | zpl_random_gen_u32(r);
    }
    
    
    isize zpl_random_gen_isize(zplRandom *r) {
    ...
    i64 zpl_random_range_i64(zplRandom *r, i64 lower_inc, i64 higher_inc) {
        u64 u = zpl_random_gen_u64(r);
        i64 i = *cast(i64 *)&u;
    ...
    isize zpl_random_range_isize(zplRandom *r, isize lower_inc, isize higher_inc) {
        u64 u = zpl_random_gen_u64(r);
    ...
    f64 zpl_random_range_f64(zplRandom *r, f64 lower_inc, f64 higher_inc) {
        u64 u = zpl_random_gen_u64(r);
        f64 f = *cast(f64 *)&u;
